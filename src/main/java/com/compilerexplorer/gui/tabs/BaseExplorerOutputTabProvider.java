...
public class BaseExplorerOutputTabProvider extends BaseExplorerUtilProvider {
    ...
    @NotNull
    private final Function<CompiledText.CompiledResult, CompiledText.AsmResult> asmGetter;
    @NotNull
    ...
    public BaseExplorerOutputTabProvider(@NotNull SettingsState state, @NotNull Tabs tab, @NonNls  @NotNull String actionId, @NotNull Function<CompiledText.CompiledResult, CompiledText.AsmResult> asmGetter_) {
        super(state, tab, actionId, AsmFileType.INSTANCE);
        asmGetter = asmGetter_;
    }
    ...
    public void provide(@NotNull DataHolder data, @NotNull TabContentConsumer contentConsumer) {
        shouldHaveRun(data).ifPresentOrElse(unusedPreprocessedText -> compiledText(data).ifPresent(compiledText -> compiledText.getCompiledResultIfGood().map(asmGetter).ifPresentOrElse(
                ...
                int[] chunkToOffset = new int[(asmResult.asm != null ? asmResult.asm.size() : 0) + 1];
                int line = 0;
                for (int i = 0; asmResult.asm != null && i < asmResult.asm.size(); ++i) {
                    CompiledText.CompiledChunk chunk = asmResult.asm.get(i);
                    ...
                    if (chunk.text != null) {
                        ...
                        if (chunk.source != null && chunk.source.file != null) {
                            String currentChunkFile = PathNormalizer.normalizePath(chunk.source.file);
                ...
                chunkToOffset[asmResult.asm != null ? asmResult.asm.size() : 0] = currentOffset;

                List<TabFoldingRegion> foldingRegions = new ArrayList<>();
                if (asmResult.labelDefinitions != null) {
                    List<Pair<Integer, String>> labelEntries = asmResult.labelDefinitions.entrySet().stream().map(e -> new Pair<>(e.getValue(), e.getKey())).sorted(Pair.comparingByFirst()).toList();
    ...
    @NotNull
    protected Optional<String> shouldHaveRun(@NotNull DataHolder data) {
